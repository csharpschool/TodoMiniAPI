...
public static class ApiMethods
{
     ...
     where TContext : DbContext where TEntity : class where TPostDto : class where TDeleteDto : class
    {
        ...
        app.MapDelete($"/api/{node}s", async (DbServiceBase<TContext> db, [FromBody] TDeleteDto dto) =>
        {
            try
            {
    ...
    public static async Task<IResult> HttpSingleAsync<TContext, TEntity, TDto>(this DbServiceBase<TContext> db, int id) where TContext : DbContext where TEntity : class, IEntity where TDto : class
    {
        var result = await db.SingleAsync<TEntity, TDto>(id);
        if (result is null) return Results.NotFound();
        return Results.Ok(result);
    }
    public static async Task<IResult> HttpGetAsync<TContext, TEntity, TDto>(this DbServiceBase<TContext> db) where TContext : DbContext where TEntity : class where TDto : class =>
        Results.Ok(await db.GetAsync<TEntity, TDto>());
    public static async Task<IResult> HttpPostAsync<TContext, TEntity, TPostDto>(this DbServiceBase<TContext> db, TPostDto dto) where TContext : DbContext where TEntity : class, IEntity where TPostDto : class
    {
        try
        {
    ...
    public static async Task<IResult> HttpPutAsync<TContext, TEntity, TPutDto>(this DbServiceBase<TContext> db, TPutDto dto) where TContext : DbContext where TEntity : class, IEntity where TPutDto : class
    {
        try
        {
    ...
    public static async Task<IResult> HttpDeleteAsync<TContext, TEntity>(this DbServiceBase<TContext> db, int id) where TContext : DbContext where TEntity : class, IEntity
    {
        try
        {
    ...
    public static async Task<IResult> HttpPostReferenceAsync<TContext, TEntity, TPostDto>(this DbServiceBase<TContext> db, TPostDto dto) where TContext : DbContext where TEntity : class where TPostDto : class
    {
        try
        {
            var entity = await db.AddAsync<TEntity, TPostDto>(dto);
